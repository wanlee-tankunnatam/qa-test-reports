# tools/ — QA Dashboard (local server)
...
> **สำคัญ:** โฟลเดอร์ `tools/` นี้คือ **ตัวเครื่องมือ (tool) อย่างเดียว** — *ไม่มี* ผลการทดสอบจริงอยู่ข้างใน
...
>
> เครื่องมือนี้ **ไม่เกี่ยวกับ** รายงาน QA ที่เผยแพร่ผ่าน GitHub Pages ใน repo นี้ (ดู [../README.md](../README.md))
> ทั้งสองส่วนแยกขาดจากกัน ไม่มีไฟล์ไหนอ้างถึงกัน
...
| ลำดับ | วิธี | ตัวอย่าง |
|------|------|---------|
| 1 | `--repo-root` flag | `python3 tools/server.py --repo-root ~/Documents/rf/realfact` |
| 2 | env var | `REALFACT_REPO_ROOT=~/Documents/rf/realfact python3 tools/server.py` |
...
> 💡 **แนะนำ layout แบบ sibling** เพื่อความสะดวก — จะรัน `python3 tools/server.py` เปล่าๆ ได้เลย:
> ```
> rf/
> ├── realfact/          ← main repo (มี test data)
> └── qa-test-reports/   ← repo นี้ (tool อยู่ใน tools/)
> ```
...
```bash
python3 tools/server.py [--port 8088] [--host 127.0.0.1] [--repo-root PATH]
```
...
- **Tool (โฟลเดอร์นี้):** version-controlled ผ่าน git ตามปกติ — แก้ feature/UI แล้วเปิด PR
...
> เป้าหมายของการแยกตัว tool ออกมา: ให้ทีม QA **อัปเดต/ปรับปรุงตัว dashboard ร่วมกันได้** โดยไม่ต้องแตะ main repo
