...
- Location: e2e/uat/membership.uat.spec.js:357:3
...
```ts
  ...
  315 |     await window.waitForLoadState('domcontentloaded').catch(() => {});
  316 |     // The window sometimes re-navigates a moment after the poll above resolves (a second reload the profile
  317 |     // fetch triggers) — the button briefly disappears/reappears, so retry the visibility wait once.
  318 |     try {
  319 |       await window.getByRole('button', PROFILE_BTN).first().waitFor({ timeout: 30_000 });
  320 |     } catch {
  321 |       await window.waitForTimeout(3000);
> 322 |       await window.getByRole('button', PROFILE_BTN).first().waitFor({ timeout: 60_000 });
      |                                                             ^ TimeoutError: locator.waitFor: Timeout 60000ms exceeded.
  323 |     }
  324 |     await window.waitForTimeout(3000);
  325 |     // A team this account never consented in stops at its consent screen — that is a precondition, not a result.
  326 |     const gate = await window.getByRole('heading', { name: /คุณเข้าร่วมพื้นที่ทำงาน/ }).isVisible().catch(() => false);
  327 |     test.skip(gate, `ยังไม่ได้ให้ความยินยอมใน "${name}" — ให้ความยินยอมด้วยมือก่อน`);
  328 |     expect((await wsState()).workspaces.find((w) => w.selected)?.name).toBe(name);
  329 |   }
  330 |   async function restoreHome() {
  331 |     await window.evaluate((id) => window.liveHost.auth.selectWorkspace(id), home.workspaceId).catch(() => {});
  332 |     await window.waitForTimeout(6000);
  333 |     await window.getByRole('button', PROFILE_BTN).first().waitFor({ timeout: 30_000 }).catch(() => {});
  334 |     expect((await wsState()).workspaces.find((w) => w.selected)?.workspaceId, 'คืนทีมเดิมไม่สำเร็จ').toBe(home.workspaceId);
  335 |   }
  336 |   async function menus() {
  337 |     await openProfileMenu();
  338 |     const members = await window.getByRole('button', { name: 'สมาชิกและบทบาท' }).isVisible().catch(() => false);
  339 |     await closeMenu();
  340 |     return { brand: (await brandLink().count()) > 0, members };
  341 |   }
  342 | 
  343 |   test('TC-MEM-D.6 (uat): บัญชีเดียวมีบทบาทต่างกันในแต่ละทีม เมนูเปลี่ยนตามทีมที่ใช้อยู่', async () => {
  344 |     expect((await wsState()).workspaces.find((w) => w.selected)?.name).toBe(OWNER_TEAM);
  345 |     expect(await menus(), `ใน "${OWNER_TEAM}" (เจ้าของ)`).toEqual({ brand: true, members: true });
  346 |     await shot(window, 'D6-owner-team');
  347 |     try {
  348 |       await switchVia(host.name);
  349 |       expect(await menus(), `ใน "${host.name}" (โฮสต์)`).toEqual({ brand: false, members: false });
  350 |       await shot(window, 'D6-host-team');
  351 |     } finally {
  352 |       await restoreHome();
  353 |     }
  354 |     expect(await menus(), 'กลับทีมเจ้าของแล้วเมนูกลับมา').toEqual({ brand: true, members: true });
  355 |   });
  356 | 
  357 |   test('TC-MEM-D.2 (uat): ผู้จัดการแบรนด์เห็น "แดชบอร์ดแบรนด์" แต่ไม่มี "สมาชิกและบทบาท"', async () => {
  358 |     try {
  359 |       await switchVia(bm.name);
  360 |       expect(await menus(), `ใน "${bm.name}" (ผู้จัดการแบรนด์)`).toEqual({ brand: true, members: false });
  361 |       await brandLink().first().click();
  362 |       await expect(window.getByRole('heading', { name: 'แดชบอร์ดแบรนด์' }).first()).toBeVisible({ timeout: 30_000 });
  363 |       await shot(window, 'D2-bm-brand-dashboard');
  364 |       await window.evaluate(() => { location.hash = '#/settings/members'; });
  365 |       await window.waitForTimeout(2000);
  366 |       await expect(window.locator(MEMBERS)).toHaveCount(0);
  367 |       await shot(window, 'D2-bm-members-url');
  368 |     } finally {
  369 |       await restoreHome();
  370 |     }
  371 |   });
  372 | 
  373 |   test('TC-MEM-D.3 (uat): โฮสต์ไม่มี "แดชบอร์ดแบรนด์" และ "สมาชิกและบทบาท" · เมนูของตัวเองใช้ได้', async () => {
  374 |     try {
  375 |       await switchVia(host.name);
  376 |       expect(await menus()).toEqual({ brand: false, members: false });
  377 |       // Typing the owner-only URL does not open it either.
  378 |       await window.evaluate(() => { location.hash = '#/settings/members'; });
  379 |       await window.waitForTimeout(2000);
  380 |       await expect(window.locator(MEMBERS)).toHaveCount(0);
  381 |       for (const [name, hash] of [['หน้าหลัก', '#/dashboard'], ['เซสชัน', '#/sessions'], ['รายงานประจำวัน', null]]) {
  382 |         await window.getByRole('link', { name, exact: true }).first().click();
  383 |         await window.waitForTimeout(2500);
  384 |         if (hash) expect(await window.evaluate(() => location.hash)).toBe(hash);
  385 |         const text = await window.locator('body').innerText();
  386 |         expect(text.length, `หน้า "${name}" ว่าง`).toBeGreaterThan(50);
  387 |         expect(TECHNICAL_LEAK.test(text), `หน้า "${name}" มี error ทางเทคนิครั่ว`).toBe(false);
  388 |         await shot(window, `D3-${hash ? hash.slice(2) : 'reports'}`);
  389 |       }
  390 |     } finally {
  391 |       await restoreHome();
  392 |     }
  393 |   });
  394 | 
  395 |   test('TC-MEM-A.6 (uat): ข้อมูลแต่ละทีมแยกกัน — แดชบอร์ดแบรนด์ของทีมเจ้าของไม่ปนไปทีมที่เป็นโฮสต์', async () => {
  396 |     await brandLink().first().click();
  397 |     await expect(window.getByRole('heading', { name: 'แดชบอร์ดแบรนด์' }).first()).toBeVisible({ timeout: 30_000 });
  398 |     await window.waitForTimeout(4000);
  399 |     const ownerText = await window.locator('main').innerText();
  400 |     const summary = (ownerText.match(/\d+\s*โฮสต์\s*·\s*\d+\s*ไลฟ์[^\n]*/) || [''])[0];
  401 |     // Live titles on the owner team's dashboard (e.g. "shop_21092026") — the fingerprints to look for elsewhere.
  402 |     const lives = [...new Set(ownerText.match(/[A-Za-z0-9_.]+_\d{8}/g) || [])];
  403 |     test.info().annotations.push({ type: 'owner-team', description: `${summary} · lives=${lives.join(', ')}` });
  404 |     await shot(window, 'A6-owner-brand-dashboard');
  405 |     try {
  406 |       await switchVia(host.name);
  407 |       await expect(brandLink()).toHaveCount(0);
  408 |       for (const hash of ['#/dashboard', '#/sessions', '#/reports']) {
  409 |         await window.evaluate((h) => { location.hash = h; }, hash);
  410 |         await window.waitForTimeout(3000);
  411 |         const text = await window.locator('body').innerText();
  412 |         for (const live of lives) expect(text, `ไลฟ์ "${live}" ของ "${OWNER_TEAM}" โผล่ในทีม "${host.name}" ที่ ${hash}`).not.toContain(live);
  413 |         await shot(window, `A6-host-${hash.slice(2)}`);
  414 |       }
  415 |       test.skip(lives.length === 0, `แดชบอร์ดของ "${OWNER_TEAM}" ไม่มีไลฟ์ให้ใช้เทียบ`);
  416 |     } finally {
  417 |       await restoreHome();
  418 |     }
  419 |   });
  420 | });
  421 | 
  422 | test.describe('ไม่มีเน็ต (จำลอง: ตัดคลาวด์ คง localhost)', () => {
```